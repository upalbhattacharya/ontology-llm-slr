...
\subsection{Conversion of bibtex to CSV}
\label{subsec:bib-to-csv}

CSV files are not provided by ScienceDirect (Section
\ref{subsubsec:sciencedirect}), ACL Anthology (Section \ref{subsubsec:acl}),
ACM Digital Library (Section \ref{subsubsec:acm}) and ArXiv (Section
\ref{subsubsec:arxiv}) sources. For theses sources,
\href{https://www.zotero.org}{Zotero} is used to convert their \verb|bibtex|
files to CSV formats.

\subsection{Concatenation and deduplication}
\label{subsec:concat-dedup}

The individual CSV files of each source is combined using the \verb|compose|
utility of \href{https://zenodo.org/records/13374508}{ASReview Datatools}



\subsection{Initial Screening: Removal of Conference Proceedings, Books, Book
