...
\begin{enumerate}
  ...
  \item \textbf{Ontology Engineering Task}: Ontology learning tasks
    ...
    classifications. OL Layer Cake-based categorization
  \item \textbf{LLMs used}
  \item \textbf{LLM method used}: zero/few-shot prompting, RAG, fine-tuning, etc.
  \item \textbf{Task(?)}: Classification, Retrieval, Generation,
    etc. \cite{du2024ShortReviewOntology} 
  \item \textbf{NLP Metrics}
  \item \textbf{Ontology Metrics}
  \item \textbf{Low-resource Domain}
  \item \textbf{Minor vs. major ontology engineering focus}: Knowledge
    base/graph construction approaches use LLMs to construct an ontology with
    which to populate the graph.
\end{enumerate}

\subsection{Analysis/Questions over extracted Data}
\label{subsec:analysis-over-data}
\begin{enumerate}
  \item \textbf{Domain-based change in LLM method over time}: Medical sciences
...
\section{First Screening}
\label{sec:first-screening}

An initial screening based on the titles and abstracts led to an initial pool
of 253 papers. While reading through the full-text versions, the following
were kept in mind for inclusion in the final list of papers.

\begin{itemize}
  \item \textbf{Evaluation}: If a paper did not possess quantification of
    their results by utilization of ontology or NLP metrics for the ontology
    task(s) they worked on, the paper was to be \textbf{excluded}. While
    papers that emphasize other aspects than metrics (e.g. human-in-the-loop
    feedback-based evaluation) are insightful, it is difficult to compare and
    analyze their performance without quantifiable metrics.
  \item \textbf{Analysis}: Papers that did have quantifiable evaluation
    metrics but do not perform additional analysis for more insights are
    \textbf{included}. While the lack of additional analysis makes it
    difficult to compare LLM performance against other papers with more
    rigorous analysis, the omission of such analysis does not take away from
    the primary task of LLM-based ontology engineering.
  \item \textbf{Explicitness of the task}: If a paper focuses on construction
    of a taxonomy or a backbone ontology but does not describe it as such, it
    is \textbf{included}. Despite the lack of clear description of the task,
    the task is, in essence, of ontology engineering and thus valuable for a
    review.
  \end{itemize}
